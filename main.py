...
for i in range(1, 4):
    ...
    if i < 3:
        for j in range(1, 4):
            target_iamges[i - 1].append(pygame.transform.scale(
...
def draw_score():
    ...
    time_text = font.render(f'Time Passed: {time_passed}', True, 'black')
...
def draw_level(coords):
    ...
    for i in range(len(coords)):
        for j in range(len(coords[i])):
            ...
            screen.blit(target_iamges[level-1][i], coords[i][j])
    return target_rects
...
def draw_menu():
    ...
    global best_ammo, best_timed, best_freeplay, time_remaining, write_values, clicked, new_coords
    game_over = False
    pause = False
...
def draw_game_over():
    global level, pause, clicked, game_over, menu, points, total_shots, time_passed, time_remaining
    if mode == 0:
...
def draw_pause():
    global level, pause, clicked, menu, points, total_shots, time_passed, time_remaining, new_coords
